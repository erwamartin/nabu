;(function ($, window, document, undefined) {
  ...
  var Pizza = {
    ...
    settings : {
      donut: true,
      donut_inner_ratio: 0.75,   // between 0 and 1
    ...
    pie : function (legend) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        if (existing_text.length > 0) {
          ...
          text.attr({
            x: 80,//cx + (r + settings.percent_offset) * Math.sin(start_angle + (angles[i] / 2)),
            y: 80,//cy - (r + settings.percent_offset) * Math.cos(start_angle + (angles[i] / 2))
          });
